...
br_step_install_samba() {
  ...
  echo -e "${YELLOW}[УСТАНОВКА]${RESET} Устанавливаем: samba samba-client krb5-user winbind"
  
  if apt-get install -y samba samba-client krb5-user winbind; then
    ok "Основные пакеты Samba установлены успешно"
  else
    warn "Некоторые пакеты не установились, продолжаем..."
  fi
  
  # Проверяем установку samba-tool
  if command -v samba-tool >/dev/null 2>&1; then
    ok "samba-tool доступен"
    info "Версия Samba: $(samba-tool --version 2>/dev/null | head -1 || echo 'информация недоступна')"
  else
    # Ищем samba-tool в других местах
    warn "samba-tool не найден в PATH, выполняем поиск..."
    local found_tool=$(find /usr -name samba-tool 2>/dev/null | head -1)
    if [ -n "$found_tool" ]; then
      ok "samba-tool найден: $found_tool"
      # Временно добавляем в PATH
      export PATH="$PATH:$(dirname "$found_tool")"
    else
      err "samba-tool не найден в системе"
  ...
  # Проверяем другие важные бинарники
  info "Проверка доступности утилит:"
  for tool in smbd nmbd winbindd kinit; do
  ...
  warn "Далее: выполните 'Provision Samba AD DC' (шаг 2 в меню BR-SRV)"
}
br_step_provision() {

...
br_step_install_samba() {
  ...
  # Пробуем установить task-samba-dc (как в рабочем скрипте)
  info "Попытка установки task-samba-dc..."
  if apt-get install -y task-samba-dc; then
    ok "task-samba-dc установлен успешно"
  else
    warn "task-samba-dc не найден, пробуем установить отдельные пакеты..."
    
    # Устанавливаем пакеты по отдельности как в рабочем скрипте
    info "Установка основных пакетов Samba..."
    apt-get install -y samba samba-client krb5-kdc krb5-admin-server winbind bind
    
    # Проверяем что установилось
    info "Проверка установленных пакетов:"
    for pkg in samba samba-client krb5-kdc krb5-admin-server winbind; do
      if rpm -q $pkg >/dev/null 2>&1; then
        echo -e "  ${GREEN}✓${RESET} $pkg установлен"
      else
        echo -e "  ${YELLOW}⚠${RESET} $pkg не установлен"
      fi
    done
  fi
  
  # Проверяем доступность samba-tool
  info "Поиск samba-tool в системе..."
  local samba_tool_path=$(find /usr -name samba-tool 2>/dev/null | head -1)
  
  if [ -n "$samba_tool_path" ]; then
    ok "samba-tool найден: $samba_tool_path"
    # Временно добавляем в PATH
    export PATH="$PATH:$(dirname "$samba_tool_path")"
  else
    # Если не нашли, пробуем установить из других пакетов
    warn "samba-tool не найден, пробуем найти подходящий пакет..."
    apt-cache search samba | grep -i tools || apt-cache search samba | grep -i admin
    
    # Пробуем установить samba-dc-tools или аналогичный
    apt-get install -y samba-dc-tools 2>/dev/null || \
    apt-get install -y samba-ad-tools 2>/dev/null || \
    apt-get install -y samba-tools 2>/dev/null || true
    
    # Повторный поиск
    samba_tool_path=$(find /usr -name samba-tool 2>/dev/null | head -1)
    if [ -n "$samba_tool_path" ]; then
      ok "samba-tool установлен: $samba_tool_path"
      export PATH="$PATH:$(dirname "$samba_tool_path")"
    else
      err "samba-tool не удалось найти или установить"
      info "Доступные samba-пакеты:"
      rpm -qa | grep samba
      return 1
    fi
  fi
  ...
  if command -v samba-tool >/dev/null 2>&1; then
    ok "samba-tool готов к использованию"
    info "Версия: $(samba-tool --version 2>/dev/null | head -1 || echo 'Samba AD DC')"
  else
    err "Критическая ошибка: samba-tool недоступен"
    return 1
  fi
  
  ok "Установка Samba завершена"
  warn "Далее: выполните 'Provision Samba AD DC' (шаг 2)"
}
br_step_provision() {

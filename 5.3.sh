...
br_step_install_samba() {
  ...
  if apt-get install -y samba-ad-dc; then
    ...
    # Устанавливаем основные пакеты Samba и Kerberos для ALT Linux
    warn "Пакет samba-ad-dc не найден, устанавливаем основные пакеты..."
    
    # Основные пакеты Samba
    apt-get install -y samba || {
      err "Не удалось установить пакет samba"
      return 1
    }
    
    # Пакеты Kerberos для ALT Linux
    apt-get install -y krb5-kdc krb5-admin-server krb5-user || {
      warn "Не удалось установить некоторые пакеты Kerberos, продолжаем..."
    }
    
    # Дополнительные пакеты
    apt-get install -y winbind bind 2>/dev/null || true
  fi
  ...
  if command -v samba-tool >/dev/null 2>&1; then
    ...
    if [ -n "$samba_tool_path" ]; then
      ...
      # Добавляем в PATH на время выполнения скрипта
      export PATH="/usr/sbin:/usr/lib/samba:$PATH"
      return 0
    else
      ...
      err "samba-tool не найден. Попробуйте установить вручную:"
      echo "apt-get install -y samba-ad-dc"
      echo "или:"
      echo "apt-get install -y samba krb5-kdc krb5-admin-server"
      return 1
    fi
  fi
}
Быстрое решение - выполни вручную на BR-SR
br_step_provision() {

...
br_step_install_samba() {
  info "BR-SRV: шаг 1 — диагностика и установка Samba"
  ensure_network_and_dns || return 1
  
  echo -e "${CYAN}=== ДИАГНОСТИКА СИСТЕМЫ ===${RESET}"
  info "Поиск доступных пакетов Samba..."
  echo "Доступные пакеты:"
  apt-cache search samba | head -10
  
  info "Уже установленные пакеты:"
  rpm -qa | grep samba
  
  echo -e "${CYAN}=== УСТАНОВКА ===${RESET}"
  apt-get update
  
  # Пробуем все возможные варианты установки
  info "Попытка 1: Установка всех samba-пакетов"
  apt-get install -y samba* --no-install-recommends
  
  info "Попытка 2: Установка конкретных пакетов"
  for pkg in samba-dc samba-ad-dc samba-server samba-common samba-client; do
  ...
  info "Поиск samba-tool..."
  find /usr -name "*samba*" -type f -executable | grep -v lib | head -10
  
  local samba_tool=$(find /usr -name samba-tool 2>/dev/null | head -1)

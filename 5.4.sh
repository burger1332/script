...
br_step_install_samba() {
  ...
  echo -e "${CYAN}=== АНАЛИЗ УСТАНОВЛЕННЫХ ПАКЕТОВ ===${RESET}"
  
  # Проверяем что уже установлено
  info "Поиск установленных пакетов Samba..."
  rpm -qa | grep -i samba
  
  # Samba уже установлен (видно из твоего вывода), нужно найти бинарники
  info "Поиск исполняемых файлов Samba..."
  echo -e "${YELLOW}Поиск в /usr:${RESET}"
  find /usr -name "*samba*" -type f -executable 2>/dev/null | head -20
  
  echo -e "${YELLOW}Поиск в /usr/sbin:${RESET}"
  ls -la /usr/sbin/samba* 2>/dev/null || echo "Нет файлов в /usr/sbin/samba*"
  
  echo -e "${YELLOW}Поиск samba-tool:${RESET}"
  find /usr -name "samba-tool" 2>/dev/null
  
  echo -e "${YELLOW}Поиск smbd:${RESET}"
  find /usr -name "smbd" 2>/dev/null
  
  # Проверяем какие команды доступны
  info "Проверка доступных команд:"
  for cmd in samba smbd nmbd winbindd; do
    if command -v $cmd >/dev/null 2>&1; then
      echo -e "  ${GREEN}✓${RESET} $cmd: $(which $cmd)"
    else
      echo -e "  ${RED}✗${RESET} $cmd: не найден"
    fi
  done

  # Если samba установлен но samba-tool нет, пробуем доустановить
  if ! command -v samba-tool >/dev/null 2>&1; then
    warn "samba-tool не найден, пробуем доустановить..."
    apt-get update
    apt-get install -y samba-dc samba-tools 2>/dev/null || true
  fi

  # Финальная проверка
  local samba_tool=$(command -v samba-tool || find /usr -name samba-tool 2>/dev/null | head -1)
  if [ -n "$samba_tool" ]; then
    ok "SUCCESS: samba-tool найден: $samba_tool"
    # Добавляем в PATH если нужно
    if [[ "$samba_tool" != /* ]]; then
      samba_tool=$(command -v samba-tool)
    fi
    export PATH="$PATH:$(dirname "$samba_tool")"
    info "Версия: $($samba_tool --version 2>/dev/null | head -1 || echo 'Samba AD DC')"
    return 0
  else
    err "FAIL: samba-tool не найден"
    echo -e "${RED}РЕШЕНИЕ:${RESET}"
    echo "1. Проверьте репозитории: cat /etc/apt/sources.list"
    echo "2. Добавьте репозиторий с Samba AD DC"
    echo "3. Или установите вручную: apt-get install samba-dc"
    return 1
  fi
}
br_step_provision() {

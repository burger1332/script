...
CONFIG_FILE="4.3.conf"
if [ -f "$CONFIG_FILE" ]; then
    echo "Загружаем конфигурацию из $CONFIG_FILE..."
    source "$CONFIG_FILE"
    log_done "Конфиг загружен!"
else
    echo -e "${RED}Ошибка: Файл $CONFIG_FILE не найден!${RESET}"
    echo "Создайте файл конфигурации или запустите скрипт в той же папке, где лежит 4.3.conf"
    exit 1
fi

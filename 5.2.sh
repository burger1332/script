...
br_step_install_samba() {
  ...
  info "Установка Samba AD DC..."
  # Устанавливаем необходимые пакеты с обработкой ошибок
  if ! apt-get install -y task-samba-dc samba smbclient krb5-user winbind; then
    err "Ошибка установки основных пакетов, пробуем альтернативный метод..."
    apt-get install -y samba samba-dsdb-modules samba-vfs-modules winbind krb5-config krb5-user || {
      err "Критическая ошибка установки Samba"
      return 1
    }
  fi
  ...
  if check_samba_installed; then
    ...
    err "Samba не установлен корректно. Проверьте вручную."
    return 1
  fi
}
